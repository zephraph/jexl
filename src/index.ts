...
// Helper function to extract parameter name from Param type
...
const builtins: Record<string, BuiltinFunction> = {
	...
	subtract: (...args: JexlValue[]) => {
		const [a, b] = args;
		if (typeof a !== "number" || typeof b !== "number") {
			throw new Error("subtract requires two numbers");
		}
		return a - b;
	},
	multiply: (...args: JexlValue[]) => {
		const [a, b] = args;
		if (typeof a !== "number" || typeof b !== "number") {
	...
	divide: (...args: JexlValue[]) => {
		const [a, b] = args;
		if (typeof a !== "number" || typeof b !== "number") {
			throw new Error("divide requires two numbers");
		}
		return a / b;
	},
	less: (...args: JexlValue[]) => {
		const [a, b] = args;
		if (typeof a !== "number" || typeof b !== "number") {
			throw new Error("less requires two numbers");
		}
		return a < b;
	},
	greater: (...args: JexlValue[]) => {
		const [a, b] = args;
		if (typeof a !== "number" || typeof b !== "number") {
			throw new Error("greater requires two numbers");
		}
		return a > b;
	},
	equals: (...args: JexlValue[]) => {
		const [a, b] = args;
		return a === b;
	},
	print: (...args: JexlValue[]) => {
		console.log(...args);
		return null;
	},
...
function evaluate(expr: Expression, env: Environment): JexlValue {
	...
	if ("function" in expr) {
		const funcDef = expr as FunctionDef;
